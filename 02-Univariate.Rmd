...
Consider a white noise process $\{\varepsilon_t\}_{t = -\infty}^{+\infty}$ (see Def. \@ref(def:whitenoise)) of variance $\sigma^2$. Process $y_t$ is a $p^{th}$-order autoregressive process (AR(p)) if its dynamics is defined by the following difference equation (with $\phi_p \ne 0$):
\begin{equation}
