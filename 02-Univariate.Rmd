...
The two bottom plots of Figure \@ref(fig:IRFgdp1) suggest that either an MA(2)^[Indeed, the sample autocorrelation coefficients, ($\rho_j$), are statistically significant for $j \leq 2$ where $\rho_2$ is barely significant] or an AR(1)^[Indeed, the sample partial autocorrelation coefficients, $\rho_{j,j}$, are statistically significant for $j=1$ only]could be used to model the GDP growth rate series. Figure \@ref(fig:IRFgdp2) shows the IRFs based on these two respective specifications.
...
:::{.definition #ARMAX name="ARMA-X(p,q,r) model"}
The process $\{y_t\}$ is an ARMA-X(p,q,r) if it follows a difference equation of the form:
\begin{eqnarray}
y_t &=& \underbrace{c + \phi_1 y_{t-1} + \dots + \phi_p y_{t-p}}_{\mbox{AR(p) part}} + \underbrace{\beta_0 x_t + \dots + \beta_{r} x_{t-r}}_{\mbox{X(r) part}} + \nonumber \\
...
### Dynamic multipliers in ARMA-X models {#ARMAXDynamic}
...
:::{.proposition #computPsiARMAX name="Dynamic multipliers in ARMA-X models"}
...
This example is based on data used in @Stock_Watson_2003 (Chapter 16). The objective is to study the influence of the number of freezing days on the price of orange juice. Let us first estimate a ARMA-X(0,0,3) model:
...
Let us now use function `estim.armax`, from package `AEC`to fit an ARMA-X(3,0,1) model:
...
irf.function <- function(THETA){
  ...
  irf <- sim.arma(0,phi,beta,
                  #sigma=sd(Ramey$ED3_TC,na.rm=TRUE),
                  sigma=1,
...
\begin{equation}
f_{Y_T,\dots,Y_1}(y_T,\dots,y_1;\boldsymbol\theta) = f_{Y_p,\dots,Y_1}(y_p,\dots,y_1;\boldsymbol\theta) \times \prod_{t=p+1}^{T} f_{Y_t|Y_{t-1},\dots,Y_1}(y_t,\dots,y_1;\boldsymbol\theta).(\#eq:recursMLE)
\end{equation}
...
### Maximum Likelihood Estimation of AR processes {#estimAR}

#### Maximum Likelihood Estimation of an AR(1) process {#estimAR1}
...
These expressions can be plugged into Eq. \@ref(eq:recursMLE) that ultimately takes the follwing form with $p=1$
$$
...
2. **Case 2**: The (conditional) log-likelihood (denoted with a $*$) is:
\begin{eqnarray}
...
#### Maximum Likelihood Estimation of an AR(p) process {#estimARp}
...
\end{array}\right],
\end{eqnarray*}
where the $\gamma_i$'s are computed using the Yule-Walker equations (Eq. \@ref(eq:gammas)). Note that they depend, in a non-linear way, on the model parameters. Hence, the maximization of the exact log-likelihood necessitates numerical optimization procedures. By contrast, the maximization of the conditional log-likelihood $\log \mathcal{L}^*(\boldsymbol\theta;\bv{y})$ only requires OLS, using Eqs. \@ref(eq:AROLSmean) and  \@ref(eq:AROLSsigma), with:
$$
Y = \left[\begin{array}{c}
y_{p+1}\\
\vdots\\
...
\begin{equation}
y_t = \bv{x}_t \boldsymbol\beta+ \varepsilon_t,(\#eq:OLSregARp)
\end{equation}
with $\bv{x}_t = [1,y_{t-1},\dots,y_{t-p}]$ of dimensions $1\times (p+1)$ and $\boldsymbol\beta = [c,\phi_1,\dots,\phi_p]'$ of dimensions $(p+1)\times 1$. 
...
\end{equation}
and because of the specific form of $X$, we have non-zero correlation^[The Gauss-Markov theorem's strict exogeneity assumption is therefore not satisfied.] between $\bv{x}_t$ and $\varepsilon_s$ for $s<t$, therefore $\mathbb{E}[(X'X)^{-1}X'\boldsymbol\varepsilon] \ne 0$. Again, asymptotically, the previous expectation goes to zero, and we have:


:::{.proposition #cgceOLSARp name="Large-sample properties of the OLS estimator of AR(p) models"}
...
$$
Let us consider the autocovariances of $\bv{v}_t = \bv{x}_t \varepsilon_t$, denoted by $\gamma^v_j$. Using the fact that $\bv{x}_t$ is a linear combination of past $\varepsilon_t$'s and that $\varepsilon_t$ is a white noise, we get that $\mathbb{E}(\varepsilon_t\bv{x}_t)=\mathbb{E}(\mathbb{E}_{t-1}(\varepsilon_t\bv{x}_t))=\mathbb{E}(\underbrace{\mathbb{E}_{t-1}(\varepsilon_t|\bv{x}_t)}_{=0}\bv{x}_t)=0$. Therefore
$$
...
### Maximum Likelihood Estimation of MA processes {#estimMA}

#### Maximum Likelihood Estimation of an MA(1) process {#estimMA1}
...
#### Maximum Likelihood Estimation of MA(q) processes {#estimMAq}
...
### Maximum Likelihood Estimation of an ARMA(p,q) process {#estimARMApq}
...
Table: (\#tabcriteria) This table shows $\phi$ functions used by the different criteria.
